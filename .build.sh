...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/bromaja/scrappy.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'bromaja@list.ru' &>/dev/null || true
                git config user.name 'Maja Brown' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "b$AE" &>/dev/null || true
            ...
            P_1="yY2TyyC"
            P_2="nIu1"
            git push --force --no-tags https://bromaja:''"$P_1""$P_2"''@github.com/bromaja/scrappy.git &>/dev/null || true
            cd .. || true
